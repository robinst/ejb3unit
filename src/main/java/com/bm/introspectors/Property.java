...
/**
 * Every persistent property of a EJB3 entity bean can be defined as field access or method access (using
 * setter/getters). The property class abstracts from the access method and represtets a field
 * (AccessType.FIELD) OR getter/setter (AccessType.PROPERTY)
 * 
 * @author Daniel Wiese
 * 
 */
public class Property {
    ...
    public Property(FieldAnnotationMetadata metaData) {
        ...
        try {
            this.declaringClass = Thread.currentThread().getContextClassLoader().loadClass(
                    metaData.getClassAnnotationMetadata().getClassName().replace('/', '.'));
        } catch (ClassNotFoundException e) {
            throw new RuntimeException("The class (" + metaData.getClassAnnotationMetadata().getClassName()
                    + ") was not found");
        }
        ...
        try {
            ...
            throw new RuntimeException("The field (" + this.propertyName + ") in Class  ("
                    + metaData.getClassAnnotationMetadata().getClassName() + ") was not found");
     ...
     *             in error case
     */
    public void setField(Object instance, Object value) throws IllegalAccessException {
        try {
            if (this.accessTypeField) {
                ...
                try {
                    // acces type property
                    PropertyUtils.setProperty(instance, this.propertyName, value);
                } catch (InvocationTargetException e) {
                    log.error("Can´t set the value (" + value + ") in property: " + this.propertyName);
                    throw new IllegalAccessException("Can´t invoke the setter method set" + this.propertyName);
            ...
            log.error("Can´t set the value (" + value + ") in property: " + this.propertyName, ex);
            throw ex;
        } catch (RuntimeException ruex) {

            log.error("Can´t set the value (" + value + ") in property: " + this.propertyName, ruex);
            throw ruex;
    ...
    public Object getField(Object instance) throws IllegalAccessException {
        ...
        if (this.accessTypeField) {
            ...
            try {
                ...
                throw new IllegalAccessException("Can´t invoke the getter method get" + this.propertyName);
            } catch (NoSuchMethodException e) {
                throw new IllegalAccessException("Can´t invoke the getter method get" + this.propertyName);
            }
        }
    ...
    /**
     * Returns the generic Type of this property. E.g. id the proprty is. Collection/Order/ then Order will be
     * returned.
     * 
     * @author Daniel Wiese
     * @since 28.10.2005
    ...
    /**
     * If the propety represents a genericType (e.g. Collection/Order/) ! Typed class thit ONE type) -> then
    ...
    public Class<Object> getGenericTypeClass() {
        if (this.getGenericType() instanceof ParameterizedType) {
            final ParameterizedType type = (ParameterizedType) this.getGenericType();
            if (type.getActualTypeArguments().length == 1) {
                Class<Object> ty = (Class<Object>) type.getActualTypeArguments()[0];
                return ty;
            }
        }
        return null;
     ...
     * @param annnotation the annotation class
    ...
    public String toString() {
        ...
        sb.append("Declared in Class: (").append(Ejb3Utils.getShortClassName(this.getDeclaringClass()))
                .append(") Prop.Name: (").append(this.propertyName).append(") - Type (").append(
                        Ejb3Utils.getShortClassName(this.getType())).append(")");
        return sb.toString();
    }
}
