...
public final class RelationPropertyResolver {
	...
	private static final Logger log = Logger
			.getLogger(RelationPropertyResolver.class);
	 ...
	 */
	public static Property findAttributeForRelationAtOtherSide(
			Class<Object> declaredInClass, String mappedBy) {
		// now look for the other if exist at the global store
		Property relProp = GlobalRelationStore.getStore().getProperty(
				declaredInClass, mappedBy);
		if (relProp == null) {
			// use a new instrospector to put the relation to the global
			// store
			final EntityBeanIntrospector<Object> tmpIn = new EntityBeanIntrospector<Object>(
					declaredInClass);
			log.debug("Dependend class: " + tmpIn.getTableName());
			// now it should be in theglobal store
			relProp = GlobalRelationStore.getStore().getProperty(
					declaredInClass, mappedBy);
			if (relProp == null) {
				throw new RuntimeException(
						"Cant´t resolve releations for property (" + mappedBy
								+ ") decared in class ("
								+ declaredInClass.getName() + ")");
			}
		}
		return relProp;
	}
	...
	public static Property findAttributeForRelationAtOtherSide(
		...
		if (relProp == null) {
			...
			if (relProp == null) {
				throw new RuntimeException(
						"Cant´t resolve releations for property ("
								+ aktProperty.getName() + ") in class ("
