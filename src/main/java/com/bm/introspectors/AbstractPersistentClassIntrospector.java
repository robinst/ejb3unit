...
public abstract class AbstractPersistentClassIntrospector<T> implements Introspector<T> {
    ...
    protected void processAccessTypeField(Class<T> toInspect) {
        ...
        do {
        	Field[] fields = clazz.getDeclaredFields();
                List<AnnotatedProperty> annotatedProperties = new ArrayList<AnnotatedProperty>(fields.length);
        	for (Field aktField : fields) {
        		...
        		if (!this.isStatic(aktField) && !aktField.getName().startsWith("$")) {
        			Annotation[] fieldAnnotations = aktField.getAnnotations();
                                annotatedProperties.add(new AnnotatedProperty(aktField, fieldAnnotations));
        		}
        	}
                this.processAnnotations(toInspect, annotatedProperties);
    ...
    protected void processAccessTypeProperty(Class<T> toInspect) {
        ...
        List<AnnotatedProperty> annotatedProperties = new ArrayList<AnnotatedProperty>(properties.length);

        for (PropertyDescriptor aktProperty : properties) {
            // dont's introspect fields generated by hibernate
            getLogger().debug("examine property=" + aktProperty.getName());
            if (!aktProperty.getReadMethod().getName().equals("getClass")) {
                ...
                getLogger().debug("process property=" + aktProperty.getName());
                annotatedProperties.add(new AnnotatedProperty(aktProperty.getReadMethod().getDeclaringClass(), aktProperty, methodAnnotations));
            }
        }
        this.processAnnotations(toInspect, annotatedProperties);
    }
    ...
    /**
     * Analyse the annotations of fields or getter methods.
     * 
     * Processing is done in two phases. During the first phase
     * are examined only the Id annotations.
     * This assures that GlobalPrimaryKeyStore will be correctly
     * inicialized before the second phase, which might need the keys
     * to perform postprocessing of relations of another inspected 
     * entity (@see com.bm.introspectors#postProcessRelationProperties()).
     * 
     * @param annotatedProperties -
     *            list of properties with annotations
     * @param classToInspect -
     *            the class to inspect
     */
    private void processAnnotations(Class<T> classToInspect, 
            List<AnnotatedProperty> annotatedProperties) {
        for (AnnotatedProperty aP : annotatedProperties) {
            processIdAnnotations(classToInspect, aP);
        }
        for (AnnotatedProperty aP : annotatedProperties) {
            processNonIdAnnotations(classToInspect, aP);
        }
    }
    
    private void processIdAnnotations(Class<T> classToInspect, 
            AnnotatedProperty aktProperty) {
        Annotation[] propertyAnnotations = aktProperty.getPropAnnotations();
    ...
    private void processNonIdAnnotations(Class<T> classToInspect, 
            AnnotatedProperty aktProperty) {
        Annotation[] propertyAnnotations = aktProperty.getPropAnnotations();
        ...
        // look into the other than id annotations
        for (Annotation a : propertyAnnotations) {
            ...
            if (a instanceof OneToMany) {
                ...
                if (aktProperty.getGenericTypeClass() != null || !void.class.equals(targetEntityType)) {
                    ...
                    Property relProp = RelationPropertyResolver.findAttributeForRelationAtOtherSide(
                            ty, mappedBy);
                    OneToManyRelation o2mReleation = new OneToManyRelation(classToInspect, ty,
    ...
    private static class AnnotatedProperty extends Property {
        
        private Annotation[] propAnnotations;

        public AnnotatedProperty(Field fieldName, Annotation[] annotations) {
            super(fieldName);
            this.propAnnotations = annotations;
        }

        public AnnotatedProperty(FieldAnnotationMetadata metaData, 
                Annotation[] annotations) {
            super(metaData);
            this.propAnnotations = annotations;
        }

        public AnnotatedProperty(Class declaringClass, 
                PropertyDescriptor property, Annotation[] annotations) {
            super(declaringClass, property);
            this.propAnnotations = annotations;
        }

        public Annotation[] getPropAnnotations() {
