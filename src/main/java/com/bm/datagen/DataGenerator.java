...
public class DataGenerator {
    ...
    /**
     * This method should be called every time before an entity creation task > this method tells every
     * generator to call his prepare method(annotation).
     */
    public void prepare() {
    ...
    /**
     * This method should be called every time before an entity creation task > this method tells every
     * generator to call his clenup method (annotation).
     */
    public void cleanup() {
        Collection<List<Generator>> tmp = this.generators.values();
        for (List<Generator> allGenerators : tmp) {
     ...
     * @param introspector -
     *            the current instrospector (belonging to the class with the field)
     * @param instance -
     *            the current instance
     * @return - the new generated value
     */
    public <T> Object getNextValue(Property property, Introspector<T> introspector, Object instance) {
        final Class type = this.getType(property);
        if (generators.containsKey(type)) {
            final List<Generator> actList = generators.get(type);
            int highestSpecialisation = -1;
            Generator<Object> choosenGen = null;
            // seatch for the best generator > highest specialisation
            for (Generator<Object> actGen : actList) {
                GeneratorType gtype = this.getGeneratorTypeAnnotation(actGen);
                int actSpecialisation = this.generatorMatcher(property, gtype, introspector);
            ...
            if (choosenGen != null && highestSpecialisation >= 0) {
                ...
                // inject
                this.injector.inject(choosenGen);

                // call the choosen generator
                return choosenGen.getValue();
            }
            log.error("The type (" + type.getName() + ") is registered, but the field (" + property.getName()
                    + ") has no generator");
            throw new RuntimeException("The type (" + type.getName() + ") is registered, but the field ("
                    + property.getName() + ") has no generator");

        }
        // if no generator is registered, the value is null
        if (log.isDebugEnabled()) {
            log.debug("The type (" + type.getName() + ") is not registered to a generator(Property:"
                    + property.getName() + ")");
        }
    ...
    /**
     * This method checks if the choosen generator should be used for this field The highest integer return
     * value will be used - the return value is a value for the specialisation of the generator
     * 
     * @param <T> -
     *            the type of the bean
     * 
     ...
     *            the current field
     * @param gtype -
     *            the current generator type annotation (belongs to thecurent generator)
     * @param introspector -
     *            the current instrospector (belonging to the class with the field)
     * @return -the specialisation metric (-1 no match, 0=basic specialization , e.g 3 high specialisation The
     *         highist specialisation of a generator will be called
     */
    private <T> int generatorMatcher(Property property, GeneratorType gtype, Introspector<T> introspector) {
        int back = -1;
        ...
        } else if (gtype.fieldType() == FieldType.PK_FIELDS && pkFields.contains(property)) {
            if (gtype.field().length == 1 && gtype.field()[0].equals(ALL_FIELDS)) {
                // all types/ pk fields
                back = 2;
            } else if (this.isFieldInList(property, gtype)) {
        ...
        } else if (gtype.fieldType() == FieldType.NON_PK_FIELDS && !pkFields.contains(property)) {
            if (gtype.field().length == 1 && gtype.field()[0].equals(ALL_FIELDS)) {
                // all types/ all fields
                back = 2;
            } else if (this.isFieldInList(property, gtype)) {
