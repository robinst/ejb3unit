...
public class DataGenerator {
	...
	public void registerGen(Generator toRegister) {
		...
		if (gtype != null) {
			if (generators.containsKey(gtype.className())) {
				final List<Generator> aktList = generators.get(gtype
						.className());
	...
	public void prepare() {
		...
		for (List<Generator> generators : tmp) {
	...
	public <T> Object getNextValue(Property property,
		...
		if (generators.containsKey(type)) {
			...
			if (choosenGen != null && highestSpecialisation >= 0) {
				...
				log.error("The type (" + type.getName()
						+ ") is registered, but the field ("
			...
			if (log.isDebugEnabled()) {
				log.debug("The type (" + type.getName()
						+ ") is not registered to a generator(Property:"
						+ property.getName() + ")");
			}
	...
	private <T> int generatorMatcher(Property property, GeneratorType gtype,
		...
		if (gtype.fieldType() == FieldType.ALL_TYPES) {
			if (gtype.field().length == 1
					&& gtype.field()[0].equals(ALL_FIELDS)) {
