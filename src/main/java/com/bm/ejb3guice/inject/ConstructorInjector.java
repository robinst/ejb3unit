...
class ConstructorInjector<T> {
	...
	final CreationListner creationListner;

	final Class<? extends Annotation>[] injectionMarkers;

	ConstructorInjector(InjectorImpl injector, Class<T> implementation) {
		this.implementation = implementation;
		injectionMarkers = (injector.injectionMarkers == null) ? Ejb3Guice
				.markerToArray(Inject.class) : injector.injectionMarkers;
		creationListner = injector.creationListener;
		...
		memberInjectors = injector.injectors.get(implementation).toArray(
				new InjectorImpl.SingleMemberInjector[0]);
	...
	private ConstructorInjector() {
		...
		creationListner = null;
		injectionMarkers = Ejb3Guice.markerToArray(Inject.class);
	}

	InjectorImpl.SingleParameterInjector<?>[] createParameterInjector(
			InjectorImpl injector, Constructor<T> constructor) {
		try {
			return constructor.getParameterTypes().length == 0 ? null // default
					// constructor.
					: injector.getParametersInjectors(constructor, constructor
	...
	private Constructor<T> findConstructorIn(InjectorImpl injector,
		...
		Constructor<T>[] constructors = (Constructor<T>[]) implementation
				.getDeclaredConstructors();
		for (Constructor<T> constructor : constructors) {
			for (Class<? extends Annotation> currentMarker : injectionMarkers) {
				Annotation inject = constructor.getAnnotation(currentMarker);
				if (inject != null) {
					if (inject instanceof Inject
							&& ((Inject) inject).optional()) {
						injector.errorHandler.handle(StackTraceElements
		...
		try {
			...
			injector.errorHandler.handle(StackTraceElements
					.forMember(implementation.getDeclaredConstructors()[0]),
	...
	Object construct(InternalContext context, Class<?> expectedType) {
		ConstructionContext<T> constructionContext = context
				.getConstructionContext(this);
		...
		try {
			...
			try {
				Object[] parameters = InjectorImpl.getParameters(context,
						parameterInjectors);
				t = constructionProxy.newInstance(parameters);
				if (this.creationListner != null) {
					this.creationListner.afterCreation(t);
				}
