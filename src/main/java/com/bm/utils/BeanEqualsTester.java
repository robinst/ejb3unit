...
public final class BeanEqualsTester extends Assert {
    ...
    public static <T> void testEqualsOnSize(List<T> original, List<T> readed) {
        ...
        if (original != null && readed != null) {
            if (original.size() != readed.size()) {
                throw new RuntimeException("Bean-Read-test: Not all/too many beans reded from the DB");
            }
        } else {
            throw new RuntimeException("Bean-Read-test: One of the collections is null");
        }
     ...
     *            the introspector
     */
    public static <T> void testEqualsOnPersistentFields(List<T> original, List<T> readed,
            ...
            for (Property akt : intro.getPersitentFields()) {
                try {
                    ...
                    if (valueOrig == null && valueRead == null) {
                        // both are null >ok
                        log.debug("Both values are null> field:" + akt.getName());
                    } else {
                        if (valueOrig == null) {
                            fail("The property (" + akt + ") is null, but the DB value is not null ("
                                    + valueRead + ")");
                        } else if (valueRead == null) {
                            fail("The DB value is null");
                        } else if (valueOrig instanceof Collection && valueRead instanceof Collection) {
                            // test collections independend from the order
                            testColletionsForEqual((Collection<? extends Object>) valueOrig,
                                    (Collection<? extends Object>) valueRead);

                        } else if (!valueOrig.equals(valueRead)) {
                            // usual equals failed
                            if (valueOrig instanceof String) {
                                printErrorForUneqalStrings((String) valueOrig, (String) valueRead);
                            }
                            fail("The values of the field (" + akt.getName() + ") are not equal");
                        }
                    }
                } catch (IllegalAccessException e) {
                    throw new RuntimeException("Can´t read the value of a field");
                }
     ...
     */
    public static <T> void testColletionsForEqual(Collection<? extends T> firstColl,
        ...
        if (firstColl == null || secondColl == null) {
            ...
            while (iterator.hasNext()) {
                ...
                if (!copyOfSecond.contains(object)) {
                    fail("Object (" + object + ") is missing in second collection (second parameter)");
                }
            ...
            if (!copyOfSecond.isEmpty()) {
                fail("Second collection has elements that aren't in the first collection: " + copyOfSecond);
            }
        } else {
            fail("The size of the collections is not equal, fist size (" + firstColl.size()
                    + "), second size (" + secondColl.size() + ")");
        }
    }

    /**
     * Test the collection for equality by calling the equals method (if not equal a runntime excetion is
     * thrown).
     * 
     * 
     * 
     * The preconditions are: We expect that the list do not contain duplicate elements - every element should
     * represent a different row in the database!<br>
     * Additional preconditions are:<br> - 2 Entitybeans representing different database-rows are NEVER
     * equal. <br> - Every bean instance representing the same DB row is always equal. <br> - 2 equal beans
     * MUST have the same hash code.<br> - N bans (e.g 30) should have different hash codes.<br>
     * 
     * @param <T> -
     *            the type of the bean
     * 
     ...
     *            the readed collection from the DB
     */
    public static <T> void testEqualsImplementationForEntityBeans(List<T> original, List<T> readed) {
        ...
        for (int i = 0; i < original.size(); i++) {
            ...
            if (origB.hashCode() != readB.hashCode()) {
                log.error("The implementation of the hash-code Method is wrong");
                log.error("Two Entity-Beans (" + origB.getClass().getName()
    ...
    private static void printErrorForUneqalStrings(String origin, String other) {
        log.error("The readed string from the database " + "is not Eqal to the origin");
